...
public class World_Cup {
    ...
    private static Subrange largestSubrangeHelper(ArrayList<Integer> array, int i, int j) {
        ...
        int mid = (i + j) / 2;
        Subrange range = compareSubranges(largestSubrangeHelper(array, i, mid), largestSubrangeHelper(array, mid+1, j), maxCrossing(array, i, j, mid));
        return range;
    }
    ...
    private static long getIncrementation(ArrayList<Integer> array) {
        long max1 = (int) Double.NEGATIVE_INFINITY;
        long max2 = (int) Double.NEGATIVE_INFINITY;
        for(int i = 0;i < array.size();i++) {
            max1 = max(max1 + array.get(i), array.get(i));
            max2 = max(max1, max2);
